...
def get_params(dist, prefix):
    ...
    if dist == "Normal":
        ...
        p["p"] = st.slider("p", 0.0, 1.0, 0.5, key=f"{prefix}_p")
    elif dist == "Binomial":
        ...
        p["p"] = st.slider("p", 0.01, 1.0, 0.5, key=f"{prefix}_p")
...
# Left Column (Dist 1)
with cols[0]:
    st.subheader("Type of Distribution")
    dist_type_1 = st.selectbox("Type of Distribution", ["Continuous", "Discrete"], key="type1")
    dist1 = st.selectbox("Distribution Want to Display", continuous_dists if dist_type_1 == "Continuous" else discrete_dists, key="dist1")
    params1 = get_params(dist1, "p1")

# Right Column (Dist 2)
with cols[2]:
    st.subheader("Type of Distribution")
    dist_type_2 = st.selectbox("Type of Distribution", ["Continuous", "Discrete"], key="type2")
    dist2 = st.selectbox("Distribution Want to Display", continuous_dists if dist_type_2 == "Continuous" else discrete_dists, key="dist2")
...
with cols[1]:
    ...
    if dist_type_1 == "Discrete":
        ax_pdf.bar(x1, pdf1, label=label1, alpha=0.6, color="salmon")
    else:
        ax_pdf.plot(x1, pdf1, label=label1, color="red", linewidth=2)
    if dist_type_2 == "Discrete":
        ax_pdf.bar(x2, pdf2, label=label2, alpha=0.6, color="gold")
    else:
        ax_pdf.plot(x2, pdf2, label=label2, color="orange", linewidth=2)
    ...
    if dist_type_1 == "Discrete":
        ax_cdf.step(x1, cdf1, where='post', label=label1, color="red")
    else:
        ax_cdf.plot(x1, cdf1, label=label1, color="blue", linewidth=2)
    if dist_type_2 == "Discrete":
        ax_cdf.step(x2, cdf2, where='post', label=label2, color="red", linestyle="dotted")
    else:
        ax_cdf.plot(x2, cdf2, label=label2, color="blue", linestyle="dotted", linewidth=2)
