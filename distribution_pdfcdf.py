...
#     xmin = min(np.min(x1), np.min(x2))
#     xmax = max(np.max(x1), np.max(x2))
#     x_common = np.linspace(xmin, xmax, 2000)

#     st.subheader("PDF/PMF of Distributions")
#     fig_pdf = go.Figure()
#     if dist_type_1 == "Discrete":
#         fig_pdf.add_trace(go.Bar(x=x1, y=pdf1, name=label1, marker_color="salmon"))
#     else:
#         pdf1_interp = np.interp(x_common, x1, pdf1)
#         fig_pdf.add_trace(go.Scatter(x=x_common, y=pdf1_interp, mode='lines', name=label1, line=dict(color="red")))

#     if dist_type_2 == "Discrete":
#         fig_pdf.add_trace(go.Bar(x=x2, y=pdf2, name=label2, marker_color="orange"))
#     else:
#         pdf2_interp = np.interp(x_common, x2, pdf2)
#         fig_pdf.add_trace(go.Scatter(x=x_common, y=pdf2_interp, mode='lines', name=label2, line=dict(color="orange")))

#     fig_pdf.update_layout(title="PDF/PMF Comparison", xaxis_title="Value", yaxis_title="Density", template="simple_white")
#     st.plotly_chart(fig_pdf, use_container_width=True)

#     st.subheader("CDF of Distributions")
#     fig_cdf = go.Figure()
#     if dist_type_1 == "Discrete":
#         fig_cdf.add_trace(go.Scatter(x=x1, y=cdf1, mode='lines+markers', name=label1, line=dict(color="red")))
#     else:
#         cdf1_interp = np.interp(x_common, x1, cdf1)
#         fig_cdf.add_trace(go.Scatter(x=x_common, y=cdf1_interp, mode='lines', name=label1, line=dict(color="blue")))

#     if dist_type_2 == "Discrete":
#         fig_cdf.add_trace(go.Scatter(x=x2, y=cdf2, mode='lines+markers', name=label2, line=dict(color="orange")))
#     else:
#         cdf2_interp = np.interp(x_common, x2, cdf2)
#         fig_cdf.add_trace(go.Scatter(x=x_common, y=cdf2_interp, mode='lines', name=label2, line=dict(color="orange")))

#     fig_cdf.update_layout(title="CDF Comparison", xaxis_title="Value", yaxis_title="Cumulative Probability", template="simple_white")
#     st.plotly_chart(fig_cdf, use_container_width=True)

def plot_distributions(x1, y1, x2, y2, label1, label2, x_label, y_label, title):
    fig = go.Figure()

    if isinstance(x1[0], (int, np.integer, float, np.floating)) and len(x1) <= 50:
        fig.add_trace(go.Bar(x=x1, y=y1, name=label1, marker_color='salmon'))
    else:
        fig.add_trace(go.Scatter(x=x1, y=y1, mode='lines', name=label1, line=dict(color='red')))

    if isinstance(x2[0], (int, np.integer, float, np.floating)) and len(x2) <= 50:
...
with cols[1]:
    ...
    plot_distributions(x1_plot, pdf1_plot, x2_plot, pdf2_plot, label1, label2, "Value", "Density", "PDF/PMF Comparison")
    ...
    plot_distributions(x1_cdf, cdf1_plot, x2_cdf, cdf2_plot, label1, label2, "Value", "Cumulative Probability", "CDF Comparison")
